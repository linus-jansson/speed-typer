    ...
    return (
        <div className="grid w-screen h-screen place-items-center" tabIndex={-1} onMouseUp={selectInput}>
            <div className="flex flex-row">
                <p className="p-2">{correct.length} / {_sentence.length}</p>
                <button className="p-2 bg-red-400 rounded-3xl font-bold" onClick={handleReset}>reset</button>
            </div>
            <div className="flex flex-wrap w-1/2">
                {/* // bg-slate-600 bg-red-500 bg-slate-300 text-slate-600 text-red-500 text-slate-300 */}
                {_sentence.split('').map((letter, index) => {
                    const isCorrect = correct.includes(index);
                    const isWrong = wrong.includes(index);
                    const bg = isCorrect ? 'slate-600' : isWrong ? 'red-500' : "";
                    const color = isCorrect ? 'slate-600' : isWrong ? 'red-500' : 'slate-300';

                    // logic for handling spaces correct and wrong
                    if (letter === ' ') {
                        return <span key={index} className={"text-3xl " + bg}>&nbsp;</span>
                    }

                    // logic for hadnling correct and wrong letters
                    return <span key={index} className={"text-3xl " + "text-" +color}>{letter}</span>
                })}
            </div>
